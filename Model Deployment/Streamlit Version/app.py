...
address = st.text_input("1. Please enter your street address followed by Toronto (eg: 7100 Woodbine Ave, Toronto)")
if address:
    geolocator = Nominatim(user_agent="haha@hotmail.com")
    location = geolocator.geocode(address)
    if location and location.longitude < -78.6 and location.longitude > -79.8 and location.latitude > 43.1 and location.latitude < 44:
        ...
        def run_result():
            ...
            if prediction <= 1600 or prediction >= 3000:
                message = "We are not so confident on our price evaluation in this case, please try evaluate rental price within range 1600 to 3000 CAD"
            elif (prediction > 1600 and prediction <= 2100) or (prediction >= 2100 and prediction < 3000):
                message = "We are very confidence that about our price evaluation within range {} CAD to {} CAD".format(
                    round((prediction - 150), 2), round((prediction + 150)), 2)
            else:
        ...
        st.write("Please enter an address within Greater Toronto Area.")
